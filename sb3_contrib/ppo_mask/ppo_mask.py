...
class MaskablePPO(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        # 将日志信息写入txt文件
        log_entries = [
            f"train/entropy_loss: {np.mean(entropy_losses):.4f}",
            f"train/policy_gradient_loss: {np.mean(pg_losses):.4f}",
            f"train/value_loss: {np.mean(value_losses):.4f}",
            f"train/approx_kl: {np.mean(approx_kl_divs):.4f}",
            f"train/clip_fraction: {np.mean(clip_fractions):.4f}",
            f"train/loss: {loss.item():.4f}",
            f"train/explained_variance: {explained_var:.4f}",
            f"train/n_updates: {self._n_updates}",
            f"train/clip_range: {clip_range:.4f}"
        ]

        if hasattr(self.policy, "log_std"):
            log_entries.append(f"train/std: {th.exp(self.policy.log_std).mean().item():.4f}")
        if self.clip_range_vf is not None:
            log_entries.append(f"train/clip_range_vf: {clip_range_vf:.4f}")

        # 写入文件：每个条目单独一行，步骤之间用空行分隔
    ...
    def learn(  # type: ignore[override]
        ...
        while self.num_timesteps < total_timesteps:
            ...
            ##############zmh add############
            import torch
            rewards_print = self.rollout_buffer.rewards.sum()
            print(rewards_print)

            # 方法2：写入日志文件
            with open(self.log_file, "a") as f:
                f.write(f"Reward: {rewards_print}\n\n")

            # # 测试测试集
            # error_evaluate = self.evaluate()

            if rewards_print > self.reward_max:
                self.reward_max = rewards_print
                self.save(self.checkpoint_path + '_' + str(self.num_timesteps) + '_' + str(rewards_print))
                np.save(self.obs_path + '_' + str(self.num_timesteps) + '_' + str(rewards_print)+'.npy', self.rollout_buffer.observations[0]) 

                self.save(self.checkpoint_path + '_best')
                np.save(self.obs_path + '_best.npy', self.rollout_buffer.observations[0]) 
            ...
            # if self.num_timesteps % (self.n_steps * 20) == 0:
            #     self.save(self.checkpoint_path + '_' + str(self.num_timesteps))
    ...
    def evaluate(self):
        ...
        for ep in range(1, total_test_episodes+1):
            ...
            reward_max = 0
            min_error = 1
            error = self.env.inference(obs)
            # print('error: ', error)

            for t in range(1, max_ep_len+1):
                action_masks = get_action_masks(self.env)
                action, _states = self.predict(obs, action_masks=action_masks)
            ...
            print('evaluate_min_error: ', min_error)
