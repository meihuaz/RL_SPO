...
from gym_rec.envs_zmh.env_25D_salinity_v2 import GridWorldEnv_25D_salinity
...
base_dir = 'save_img/GridWorldEnv_25D_t/'+str(grid_size[0])+"_"+str(grid_size[1])+'_'+str(num_agents)+'/'
if not os.path.exists(base_dir):
    os.makedirs(base_dir)

env = GridWorldEnv_25D_salinity(num_agents=num_agents, n_basis_modes=n_basis_modes, grid_size=grid_size, ep_length=200)


# model = MaskablePPO.load('/public/home/yangnan/zhaomeihua/code/stable-baselines3-contrib-master_v2/log/GridWorldEnv_25D_salinity/30_40_10/checkpoint_192000_1321.9973', env=env)
model = MaskablePPO.load('/public/home/yangnan/zhaomeihua/code/stable-baselines3-contrib-master_v2/log/GridWorldEnv_25D_salinity/30_40_5/checkpoint_684000_1221.544', env=env)
...
for ep in range(1, total_test_episodes+1):
    ep_reward = 0
    # obs, _ = env.reset()
    # obs = np.load("/public/home/yangnan/zhaomeihua/code/stable-baselines3-contrib-master_v2/log/GridWorldEnv_25D_salinity/30_40_10/obs_192000_1321.9973.npy")  
    obs = np.load("/public/home/yangnan/zhaomeihua/code/stable-baselines3-contrib-master_v2/log/GridWorldEnv_25D_salinity/30_40_5/obs_684000_1221.544.npy")  
    env.state = obs[0]
    reward_max = 0
    min_error = 1
    best_state = 0
    error_t = env.inference(obs[0])
    ...
    reconstructed_X = env.s_data_orig
...
Xt_reconstructed_test = reconstructed_pre*36
gt_Xt = reconstructed_X*36
...
if visu:
    ...
    for i in range(2):
        save_dir = 'save_img/temperature_25Ds_agent5/' + str(i) + "_fig.png"
        if not os.path.exists('save_img/temperature_25Ds_agent5/'):
            os.makedirs('save_img/temperature_25Ds_agent5/')

        # 将输入和目标图像转换为 numpy 数组
        gen_image = reconstructed_pre[i][:, :, 0]
        input_image = reconstructed_X[i][:, :, 0]
