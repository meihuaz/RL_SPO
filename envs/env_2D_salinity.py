...
class GridWorldEnv_2D_salinity(gym.Env):
    ...
    def __init__(self, ep_length=100, n_basis_modes=3, num_agents=10, grid_size= (30, 40)):
        super(GridWorldEnv_2D_salinity, self).__init__()
    ...
    def self_init(self):
        ...
        #########################加载训练集##########################
        data = np.load("/public/home/yangnan/zhaomeihua/data/Temperature_sanlity/remote/sss/train_nw_pacific.npy")
        data = (data-26.0)/(39.0-26.0)
        ...
        #########################加载测试集##########################
        # data = np.load("H:/Data/NOAA_dataset/oisst_highres_nw_pacific/train.npy")
        data_test = np.load("/public/home/yangnan/zhaomeihua/data/Temperature_sanlity/remote/sss/test_nw_pacific.npy")
        data_test = (data_test-26.0)/(39.0-26.0)
        data_test[np.isnan(data_test)] = 0
        data_test = torch.from_numpy(data_test)
        data_test = data_test.unsqueeze(-1)
        data_test = torch.flip(data_test, dims=[1])
        ...
        # 2. 初始化全零矩阵
        Xt_reconstructed = np.zeros(X.shape, dtype=np.float32)

        # 3. 将有效数据填充到对应位置
        Xt_reconstructed[:, valid_indices] = pre
        Xt_reconstructed = Xt_reconstructed.reshape((self.data_orig.shape[0], self.data_orig.shape[1], self.data_orig.shape[2]))
        
        # data_orig_denormalized = self.data_orig.numpy() * (39.0 - 26.0) + 26.0
        # Xt_reconstructed_denormalized = Xt_reconstructed * (39.0 - 26.0) + 26.0
        # error1 = np.sqrt(np.mean((data_orig_denormalized - Xt_reconstructed_denormalized) ** 2))
        # print('QR分解选择传感器位置,训练集盐度重建误差: ', error1)


        error1 = np.sqrt(np.mean((self.data_orig.numpy() - Xt_reconstructed) ** 2))
        print('QR分解选择传感器位置,训练集盐度重建误差: ', error1)
        ...
        data_test_orig_denormalized = self.data_test_orig.numpy() * (39.0 - 26.0) + 26.0
        Xt_reconstructed_test_denormalized = Xt_reconstructed_test * (39.0 - 26.0) + 26.0
        error1 = np.sqrt(np.mean((data_test_orig_denormalized - Xt_reconstructed_test_denormalized) ** 2))
        print('QR分解选择传感器位置,测试集盐度重建误差: ', error1)
    ...
    def plot(self, state):
        ...
        # 2. 初始化全零矩阵
        Xs_reconstructed_test = np.zeros(self.X_test.shape, dtype=np.float32)

        # 3. 将有效数据填充到对应位置
        Xs_reconstructed_test[:, self.valid_indices] = pre
        Xs_reconstructed_test = Xs_reconstructed_test.reshape((self.data_test_orig.shape[0], self.data_test_orig.shape[1], self.data_test_orig.shape[2]))
        error1 = np.sqrt(np.mean((self.data_test_orig.numpy()*36 - Xs_reconstructed_test*36) ** 2))
        # print('QR分解选择传感器位置,温度重建误差: ', error1)


        return error1, Xs_reconstructed_test, sensors_global
    ...
    def inference(self, state):
        ...
        error1 = np.sqrt(np.mean((data_test_orig_denormalized - Xs_reconstructed_test_denormalized) ** 2))
        # print('QR分解选择传感器位置,温度重建误差: ', error1)
